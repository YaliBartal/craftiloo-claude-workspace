# Profile

## Business Type
**Family-owned e-commerce business** since 2017

## Working Style

### Organization Approach
- **Structured departmental divisions** - 10 distinct departments with clear responsibilities
- **Task-based workflow** - Everything tracked and managed systematically
- **Priority-driven** - Clear high/medium/low priority system
- **Documentation-first** - Extensive use of Notion for instructions and knowledge base

### Communication Preferences
- **Direct communication:** Slack for team discussions
- **Email management:** Direct email (no centralized platform)
- **Documentation:** Notion for all instructions, databases, and workfiles
- **Structured meetings:** Tracked in databases with clear agendas

### Task Management
- **Platform:** Asana as primary task manager
- **Status tracking:** Clear stages from Not Started to Completed
- **Due dates:** Accountability through deadline management
- **Tagging system:** Multi-dimensional categorization (Work, Urgent, Important, etc.)

### Productivity Practices
- **Time tracking:** Toggl for productivity monitoring
- **File organization:** Google Drive for centralized file management
- **Knowledge base:** Notion as central source of truth

## Goals

### Business Focus Areas
- **Product quality:** Premium, safe materials only - no compromises
- **Customer satisfaction:** Focus on positive customer experiences (proven track record of hundreds of thousands of satisfied customers)
- **Educational value:** Products must support child development and confidence building
- **Market expansion:** US-focused multi-marketplace strategy (Amazon + retail channels)
- **Continuous improvement:** Iterative optimization through split testing

### Current Priorities (2026)

**Branding & Storefront**
- Improve overall branding
- Create a brand book
- Launch new Amazon store design
- Build a strong Barbie page on website

**Channel Expansion**
- Retail marketplaces: Target.com, Walmart.com, Michaels, Hobby Lobby, Bed Bath & Beyond
- Physical retail store placement (actively pursuing)

**Product Development - Barbie Kits**
- 8 new Barbie kits launching (currently shipping to Amazon)
- Develop 4 additional Barbie kits
- Create ~4 new crafty non-branded kits

**Financial**
- Cash flow management (challenge due to Barbie growth)
- Secure funding for Christmas order inventory

**Social Media**
- Hire social media experts
- Build presence on Facebook and Instagram
- Explore TikTok presence

## Product Philosophy

### Child-Centric Design
- **Achievable projects** - Build confidence through completion
- **Educational value** - Learning through creative problem-solving
- **Complete solutions** - Everything included, ready to use
- **Safety first** - Only high-quality, safe materials

### Quality Standards
- Premium quality products exclusively
- Rigorous safety standards
- Customer-validated designs (positive review focus)
- Iterative improvement based on feedback

## Operational Preferences

### Multi-Channel Strategy
- **Amazon US** as primary sales channel
- **Retail marketplace expansion (2026):** Target.com, Walmart.com, Michaels, Hobby Lobby, Bed Bath & Beyond
- Product tracking across all channels (ASIN, stock levels, keyword rankings)

### Inventory Management
- **Stock categories:** Optimal, Understocked, (Overstocked)
- **Product lifecycle tracking:** Established, Development, Launch, Growth
- Proactive stock optimization

### Decision-Making Style
- **Data-driven:** Split testing and analytics
- **Quality-focused:** Never compromise on materials or safety
- **Customer-centric:** Reviews and satisfaction as key metrics
- **Strategic:** Long-term planning (2026 Plans Diagram)

## Notes for Claude

### What I value
- **Efficiency:** Streamlined workflows and automation where possible
- **Quality:** High standards in everything we do
- **Family business values:** Long-term relationships and customer trust
- **Continuous learning:** Always improving processes and products

### Communication Expectations
- Be concise and direct (as per CLAUDE.md)
- Lead with answers, then context
- Use structured formats (bullets, tables, headers)
- Skip filler phrases

### Automation Opportunities
- Web scraping for competitor research (Apify)
- Task synchronization across platforms
- Product data management and updates
- Workflow optimization and process automation

---

*This profile helps Claude understand your working style, business values, and preferences for more aligned assistance.*
