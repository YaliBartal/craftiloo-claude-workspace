# PPC Portfolio Action Plan — Run Log Archive (March 2026)

### Run: 2026-03-10 — 4 Flowers Embroidery (Portfolio Rebuild)
**Target:** 4 Flowers Embroidery (110971277744354) — 9 ENABLED → 7 after rebuild
**Result:** Success (12 actions executed: 5 pauses, 3 re-enables, 1 TOS/budget adjust, 1 budget decrease, 34 negatives added)

**What happened:**
- First full deep dive for this portfolio. Health 3/10, 30d ACoS 69.8% (target 30%).
- User requested strategic rebuild plan instead of just cutting. Led to SK-dominant architecture redesign.
- Found 35 PAUSED campaigns — a graveyard of a year of enable/pause cycles. This explains the chronic issues.
- All 3 async reports (campaigns, search terms, placements) stuck PENDING — repeat issue x9+.
- Used cached 14d search terms (Mar 2), weekly snapshot (Mar 8), rank radar (Mar 9), Seller Board 7d as fallback.
- Discovered "SK embroidery kits for adults" campaign already existed paused (525516345508137) with $30/d budget and 210% TOS. Re-enabled with sane settings instead of creating new.
- Both re-enabled SK campaigns (145201311633066, 404885192371908) were on AUTO_FOR_SALES — changed to LEGACY_FOR_SALES for cost control.
- TOS on SK embroidery for beginners was still 161% in API despite tracker saying 20% (Mar 3 change). Fixed during re-enable by setting all placement bidding explicitly.

**What worked:**
- Strategic conversation with user about portfolio philosophy (seasonal vs rebuild vs organic-first) before executing
- Pulling paused campaign list revealed existing campaigns that could be re-enabled — avoided creating duplicates
- Proactive negatives on Auto (cross-craft terms: cross stitch, latch hook, sewing, knitting, crochet, diamond painting) — good preventive measure

**What didn't work:**
- Reports PENDING again (x9+ across this project's lifetime). Had to rely on 8-day-old cached data.
- Campaign IDs from different sources don't always match — weekly snapshot uses different IDs than list_sp_campaigns API. Confusing.

**Key lesson:**
- **ALWAYS check paused campaigns before creating new ones.** The "embroidery kits for adults" campaign already existed. Creating a duplicate would have caused cannibalization and confusion.
- **AUTO_FOR_SALES bidding strategy is dangerous for underperforming campaigns.** Amazon bids UP when it thinks conversion is likely, but for campaigns that aren't converting, this just burns more money. Always switch to LEGACY_FOR_SALES when re-enabling previously-bleeding campaigns.
- **TOS changes may not persist when campaigns are paused.** The Mar 3 TOS change (161%→20%) didn't take effect — API still showed 161%. Always verify and re-apply settings when re-enabling.

**Tokens/cost:** ~150K tokens (long session — strategic planning + data collection + execution)
