# PPC Agent — Run Log Archive (March 2026)

### Update: 2026-03-11 — Slack Notifications Optimized for Efficiency
**Change:** Rewrote Step 8e to be self-contained (no file reads). Removed redundant notification steps from all 11 PPC sub-skill SKILL.md files.
**Why:** Original Step 8e said "Read notification-hub SKILL.md" (609 lines) per sub-skill. In a 4-skill session, that's ~2,400 lines of redundant reads. The agent already has results in context — it doesn't need a template.
**What changed:**
- Step 8e: removed notification-hub read instruction, added inline channel routing + format rules + alert escalation
- 11 sub-skills: replaced "Read notification-hub → Recipe: X" with "Skip — orchestrator handles this"
- Non-PPC skills (listing-manager, competitor tracker, etc.): untouched, still reference notification-hub normally
**Test:** `#claude-ppc-updates` channel confirmed working (2026-03-10 test post).

---

### Update: 2026-03-10 — Slack Notifications Made Mandatory
**Change:** Added Step 8e (Post Slack Notification) as mandatory after every sub-skill run. Added Global Rule #5 enforcing Slack posts. Added Step 9c verification checkpoint.
**Why:** Notifications were fully designed in notification-hub recipes and referenced in all 8 sub-skill SKILL.md files, but never actually executed. Zero Slack posts across all historical runs. Management needs visibility into PPC activity via `#claude-ppc-updates` and `#claude-alerts`.
**Test:** Successfully posted test message to `#claude-ppc-updates` — channel connected.
**Going forward:** Every sub-skill completion must post its notification-hub recipe to Slack. Session wrap-up verifies all were sent. LESSONS.md entries must include "Slack: N/N notifications posted".

---

### Run: 2026-03-10 (v2.2 — Phases C+D+E: Revenue Tiers, Cleanup, Listing Coordination)
**Task:** Bootstrap revenue tiers, remove dead field, add listing change tracking
**Result:** Success

**Phase C — Revenue Tier Bootstrap:**
- Fetched 30d Sales Detailed (2,006 rows), mapped to 17 portfolios via sku-asin-mapping.json
- T1 (weekly) = 11, T2 (biweekly) = 2, T3 (monthly) = 4. Revenue rotation is now functional.
- 4 T3 portfolios at $0 (structural/new products). Will auto-correct on next monthly refresh.

**Phase D — State Field Cleanup:**
- Removed `organic_momentum` from 17 portfolios (unused, TACoS grade covers same signal)
- Updated Optimization Scan check #5 to use `tacos` + `tacos_grade` instead
- `metric_history` cap: 90 -> 30 (2+ months weekly data retained)

**Phase E — Listing Change Tracking:**
- Added `last_listing_change` + `listing_stage` to portfolio_index schema
- 14-day cooldown guard on listing-manager queue rules (prevents re-auditing just-optimized listings)
- Updated listing-manager SKILL.md (writes after push) + listing-ab-analyzer SKILL.md (reads for date)
- Backfilled 7 portfolios from existing `listing_push` data

**Bonus:** Fixed 22 corrupted Unicode sequences in agent-state.json (smart quotes + double-encoded em dashes from prior Python writes)

**Lesson learned:**
- Don't create state fields for skills that haven't run yet (`organic_momentum` was dead from day 1)
- Always validate JSON after Python script writes
- Seller Board 30d data maps cleanly to portfolios — schedule as monthly refresh
