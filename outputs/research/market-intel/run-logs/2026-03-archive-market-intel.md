# Daily Market Intel — Run Log Archive (March 2026)

> Extracted from `.claude/skills/daily-market-intel/LESSONS.md`. Run entries only — Known Issues, Repeat Errors, and Resolved Issues remain in the original file.
...
- [ ] Fetch Seller Board 7-day dashboard + per-ASIN detailed — FAILED 401 (x4 consecutive)

**Result:** Partial — 5/5 non-SB data sources succeeded. SB 401 again (fourth consecutive run).
...
**Is this a repeat error?** Seller Board 401 = Repeat Error x4 (escalating — must fix). saswave BSR null = RESOLVED (field confirmed as bestsellerRanks[0].rank). B0DC69M3YD rank crisis ongoing (day 27+). B08FYH13CL instability continuing. B08DDJCQKF no Rank Radar = still not configured.
...
- [ ] Fetch Seller Board 7-day dashboard + per-ASIN detailed — FAILED 401 (x3 consecutive)
- [x] Compile full report with mandatory format
- [x] Save snapshot

**Result:** Partial — 5/5 non-SB data sources succeeded. SB 401 again (third consecutive run).
...
**Is this a repeat error?** Seller Board 401 = Repeat Error x3 (escalating). saswave BSR null = NEW issue (first occurrence of this specific variant — saswave worked fine in 2026-02-28 and 2026-03-02 runs). Investigate field mapping next run.
...
- [ ] Fetch Seller Board 7-day dashboard + per-ASIN detailed — FAILED 401 (x2 consecutive)
- [x] Compile full report with mandatory format
- [x] Save snapshot

**Result:** Partial — 5/5 non-SB data sources succeeded. SB 401 again (second consecutive run).
...
**Is this a repeat error?** Seller Board 401 = Repeat Error x2 (escalating — move to Repeat Errors). B08FYH13CL instability = Repeat Error x7+. B08DDJCQKF no Rank Radar = was flagged as user requirement on 2026-03-02 but still not configured.
...
**Result:** Partial — 5/6 data sources fetched. Seller Board unavailable (401).

**What happened:**
- Apify: 9/9 keywords returned data (2nd consecutive perfect scan). No 402 errors. "kids embroidery kit" resolved (was empty x2 consecutive).
...
- "kids embroidery kit" empty x2 resolved on its own — the carry-forward approach was correct. Axesso actor is reliable overall.
...
- **axesso still doesn't detect badges** — 0 badges detected. Carried forward 3 badges from Feb 28 snapshot (B09X55KL2C OP x2, B09THLVFZK OP x1).
- B07D6D95NG still not found in any DD niche (ongoing).
...
- B0DJQPGDMQ: NEW unknown competitor at #1 in DD Latch Hook niche (1,271 sales/mo, 4.7 stars, 419 reviews)
...
**Is this a repeat error?** B08FYH13CL rank instability (x4 days). Apify empty results (x2 consecutive runs).
...
**Is this a repeat error?** Apify empty results is new. B08FYH13CL rank instability is a repeat pattern (x3 days).
...
**What didn't work:**
- B09HVSLBS6 still returns no competitive pricing (repeat issue x3)
...
**Is this a repeat error?** Yes — B09HVSLBS6 pricing (x3), Latch Hook niche stale (x2)
...
**Is this a repeat error?** Yes — FBA inventory truncation (x2) and B09HVSLBS6 pricing (x2)
