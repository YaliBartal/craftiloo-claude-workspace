"""
Daily SP-API data fetch for daily market intel.
Fetches: catalog (BSR), pricing, inventory, yesterday's orders.
"""
...
async def fetch_inventory(client: httpx.AsyncClient) -> dict:
    ...
    for s in summaries:
        ...
        if asin:
            result[asin] = total
    # Fill zeros for any missing ASINs
    for a in HERO_ASINS:
        if a not in result:
            result[a] = 0
    return result
...
async def main():
    ...
    output = {
        ...
        "orders_yesterday": {"date": "2026-03-06"},
        "hero_products": {},
        "errors": [],
    }
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        # Phase B: Pricing — serial
        print("Phase B: Pricing (serial)...")
        price_map = {}
        for asin in HERO_ASINS:
            result = await fetch_pricing(client, asin)
            price_map[asin] = result.get("price")
            if result.get("error"):
                errors.append(f"Pricing {asin}: {result['error']}")
            await asyncio.sleep(0.6)  # serial + delay for rate limit

        # Phase C: Inventory
        print("Phase C: Inventory...")
        ...
        # Phase D: Orders
        print("Phase D: Orders...")
        orders = await fetch_orders(client)
        if "error" in orders:
            errors.append(f"Orders: {orders['error']}")
        else:
            output["orders_yesterday"].update(orders)

    # Assemble hero_products
    for asin in HERO_ASINS:
        prod = {}
        cat = bsr_map.get(asin, {})
        if cat.get("bsr") is not None:
            prod["bsr"] = cat["bsr"]
        if cat.get("bsr_category"):
            prod["bsr_category"] = cat["bsr_category"]
        if cat.get("subcategory_rank") is not None:
            prod["subcategory_rank"] = cat["subcategory_rank"]
        if cat.get("subcategory"):
            prod["subcategory"] = cat["subcategory"]
        ...
        prod["stock"] = stock
        if stock == 0:
            prod["stock_note"] = "FBA=0 (verify SC)"
        output["hero_products"][asin] = prod
